...
import { questionFromUrl, updateQuestionUrl } from './share';
...
function App() {
  ...
  const [card, setCard] = useState<Question | null>(() => questionFromUrl(window.location.href));
  const [sharedArrival, setSharedArrival] = useState(() => Boolean(questionFromUrl(window.location.href)));
  const [number, setNumber] = useState(() => questionFromUrl(window.location.href) ? 1 : 0);
  const [deeper, setDeeper] = useState(false);
  const homeHeading = useRef<HTMLHeadingElement>(null);
  function start(category: Category | 'Surprise') { const chosen = drawQuestion(category); updateQuestionUrl(chosen.id); setSharedArrival(false); setMode(category); setCard(chosen); setNumber(1); setDeeper(false); window.scrollTo({top:0}); }
  function next() { const chosen = drawQuestion(mode, card?.id); updateQuestionUrl(chosen.id); setSharedArrival(false); setCard(chosen); setNumber(n=>n+1); setDeeper(false); window.scrollTo({top:0, behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'instant' : 'smooth'}); }
  function home() { updateQuestionUrl(); setSharedArrival(false); setCard(null); setDeeper(false); requestAnimationFrame(()=>homeHeading.current?.focus()); }
  return <div className="app"><header className="site-header"><button className="wordmark" onClick={home} aria-label="Ponder 首頁 / Home">PONDER <span>🌷</span></button><span className="header-note">把想太多，變成更懂彼此。<span className="inline-en" lang="en">Turn overthinking into perspective.</span></span><span className="header-spark" aria-hidden="true">✳</span></header>
    <main>{card ? <section className="play-screen"><nav className="play-nav" aria-label="遊戲操作 / Game controls"><button className="text-button" onClick={home}>← 返回 <span lang="en">Back</span></button><button className="text-button" onClick={next} aria-label="隨機換一題 / Shuffle to another question">⤨ 換一題 <span lang="en">Shuffle</span></button></nav>{sharedArrival && <p className="shared-arrival">有人想聽聽你嘅想法。<span className="inline-en" lang="en">Someone wants to hear your perspective.</span></p>}<div key={number + '-' + card.id} className="card-enter"><QuestionCard card={card} number={number} deeper={deeper} onDeeper={()=>setDeeper(true)} onNext={next}/><ShareCard card={card}/></div><p className="play-note">慢慢想，精彩的部分在對話裡。<span lang="en" className="inline-en">Take your time. The good part is the conversation.</span></p></section> : <section className="home-screen"><div className="intro"><span className="eyebrow intro-label">讓每一個想法，都有相遇的可能</span><h1 ref={homeHeading} tabIndex={-1}>多想一點，<br/><span>靠近一點。</span></h1><p className="home-tagline" lang="en">Make real connections.</p><p>把腦海裡的千迴百轉，聊成看世界的新角度。<span className="inline-en" lang="en">Turn overthinking into perspective. Turn conversation into connection.</span></p><div className="hero-art"><Tulip/><span className="art-caption">一起聊聊 <span lang="en">let’s get into it</span></span></div><span className="tiny-star" aria-hidden="true">✧</span></div><div className="pick-label"><h2>今天想聊甚麼？<span lang="en">What’s the mood?</span></h2><span>{questions.length} 題 <span lang="en">questions</span></span></div><div className="category-grid">{categories.map(category=><button key={category} className={`category-tile ${category.toLowerCase()}`} onClick={()=>start(category)}><span className="category-emoji" aria-hidden="true">{moods[category].emoji}</span><span className="category-copy"><strong>{moods[category].name} <span lang="en">{category}</span></strong><span>{moods[category].description}<span className="inline-en" lang="en">{moods[category].en}</span></span></span><span className="category-count">{questions.filter(q => q.category === category).length} 題 <span lang="en">cards</span></span></button>)}</div><button className="surprise" onClick={()=>start('Surprise')}><span>🎲</span><span>隨機選一題 <span lang="en" className="button-en">Surprise Me</span></span><span className="surprise-note">交給宇宙決定<span lang="en" className="inline-en">Leave it to the universe</span></span><span className="surprise-arrow" aria-hidden="true">→</span></button><div className="how-to"><span>選個分類<span lang="en" className="inline-en">Pick a mood</span></span><i>·</i><span>把情境讀出來<span lang="en" className="inline-en">Read it out loud</span></span><i>·</i><span>聽聽彼此的想法<span lang="en" className="inline-en">See where it goes</span></span></div></section>}</main><footer><span>多一個角度，多一分理解。<span lang="en" className="inline-en">More perspectives. Deeper connections.</span></span><span className="footer-flower" aria-hidden="true">✿</span><span>為相聚而設。<span lang="en" className="inline-en">Made for being together.</span></span></footer></div>;
}
